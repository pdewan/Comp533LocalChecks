...
@MaxValue(5)
@Explanation("Checks file <eqf.out> to see if entry queue is not emptied in FIFO order")
public class EntryQueueNotFIFO extends MonitorQueueTest{
	...
	public TestCaseResult test(Project aProject, boolean autoGrade)
		...
		try {
			...
			boolean aResult = LogGrader.testThatEntryQueueIsNotQueue(aFullFileName);
			if (aResult) {
				return pass();
			}
