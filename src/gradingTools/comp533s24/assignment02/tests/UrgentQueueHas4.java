...
@MaxValue(5)
@Explanation("Checks file <uq4.out> to see if the urgent queue ever had at least 4 elements")
public class UrgentQueueHas4 extends MonitorQueueTest{
	private String fileName = "uq4.out";
	int size = 4;
	...
	public TestCaseResult test(Project aProject, boolean autoGrade)
		...
		try {
			...
			int aSize = LogGrader.maxUrgentQueueSize(aFullFileName);
			...
			if (aResult) {
				...
				return partialPass(score, "Max urgent queue size " + aSize + " instead of " + size);
			}			
